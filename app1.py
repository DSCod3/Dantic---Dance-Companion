...
if st.session_state.downloaded_video_path:
    video_cap = cv2.VideoCapture(st.session_state.downloaded_video_path)        # video capture, first image captured
else:
    video_cap = None
    dummy_video_frame = np.zeros((480,640,3), dtype=np.uint8)       #  if no first image frame

# Open the webcam.
webcam_cap = cv2.VideoCapture(0)
if not webcam_cap.isOpened():
    st.write("Webcam not found.")
    webcam_cap = None       # same 
    dummy_webcam_frame = np.zeros((480,640,3), dtype=np.uint8)      # same
...
while (frame_v <= videoLength):
    if video_cap is not None:
        ret_v, frame_v = video_cap.read()    # ret_v = , frame_v = raw current frame 
        if not ret_v:
...
while True:
    ...
    # Adjust sleep for roughly x FPS.
    time.sleep(1/fps)
if expected_coords is not None: # expected coords are local in the loop
